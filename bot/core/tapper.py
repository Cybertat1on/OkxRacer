...
class Tapper:
            ...
            if not self.tg_client.is_connected:
                try:
                    ...
                    if not start_command_found:
                        ...
                        link = choices([settings.REF_ID, get_link_code()], weights=[60, 40], k=1)[0]
    ...
    async def get_info_data(self, http_client: aiohttp.ClientSession):
        try:
            ...
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error when getting user data: {error}")
        ...
        try:
            ...
            for task in tasks:
                if task['state'] == 0 and task['id'] != 5 and task['id'] != 9:
                    logger.info(f"<cyan>{self.session_name}</cyan> | Performing task <lc>{task['context']['name']}</lc>...")
                    response_data = await self.perform_task(http_client=http_client, task_id=task['id'])
                    if response_data:
                        logger.success(f"<cyan>{self.session_name}</cyan> | ✔️Task <lc>{task['context']['name']}</lc> completed! | "
                                       f"Reward: <e>+{task['points']}</e> points")

                    await asyncio.sleep(delay=randint(5, 10))

        except Exception as error:
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error when completing tasks: {error}")
            await asyncio.sleep(delay=3)
    ...
    async def perform_task(self, http_client: aiohttp.ClientSession, task_id: str):
        try:
            ...
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error while check in task {task_id} | Error: {e}")
    ...
    async def get_boosts(self, http_client: aiohttp.ClientSession):
        try:
            ...
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error while getting boosts | Error: {e}")
    ...
    async def buy_boost(self, http_client: aiohttp.ClientSession, boost_id: int, boost_name: str) -> bool:
        try:
            ...
            if response_json.get('code') == 0:
                logger.success(f"<cyan>{self.session_name}</cyan> | ✔️Successful purchase <lc>{boost_name}</lc>")
                return True

            return False

        except Exception as e:
            logger.error(f"<cyan>{self.session_name}</cyan> | ❌ Unknown error while buying boost: <lc>{boost_id}</lc>| Error: {e}")
    ...
    async def get_price(self, http_client: aiohttp.ClientSession):
        try:
            ...
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error when getting price: {error}")
            await asyncio.sleep(delay=3)
    ...
    async def make_assess(self, http_client: aiohttp.ClientSession):
        try:
            ...
            if response_json.get('code') == 499004:
                logger.warning(f"<cyan>{self.session_name}</cyan> | ⏳ Authorization error | Refreshing token.")
                return None
            ...
            if response_data["won"]:
                ...
                logger.success(f"<cyan>{self.session_name}</cyan> | ✔️Successful prediction | Got <yellow>{added_points}</yellow> points | "
                               f"Balance: <e>{response_data['balancePoints']}</e> | "
                               f"Chances: <m>{response_data['numChance']}</m> | "
                               f"Combo: <m>x{response_data['curCombo']}</m>")
            else:
                logger.info(
                    f"<cyan>{self.session_name}</cyan> | Wrong prediction | Balance: <yellow>{response_data['balancePoints']}</yellow> |"
                    f" Chances: <m>{response_data['numChance']}</m>")

            return response_data

        except Exception as error:
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error when making assess: {error}")
            await asyncio.sleep(delay=3)
        ...
        while True:
            try:
                if time() - access_token_created_time >= token_live_time:
                    ...
                    logger.info(f"<cyan>{self.session_name}</cyan> |💰 Balance: <yellow>{balance}</yellow>")
                ...
                for boost in boosts:
                    ...
                    if boost_id == 2 or boost_id == 3:
                        if self.can_buy_boost(balance, boost):
                            ...
                            if result:
                                logger.info(f"<cyan>{self.session_name}</cyan> | <lc>{boost_name}</lc> upgraded to "
                                            f"<m>{boost['curStage'] + 1}</m> lvl")

                if chances == 0 and refresh_time > 0:
                    logger.info(f"<cyan>{self.session_name}</cyan> | Refresh chances | 💤 Sleep <red>{refresh_time}</red> seconds")
                ...
                for _ in range(chances):
                    ...
                    if response_data is None:
                        ...
                        if combo_count and combo_count >= settings.MAX_COMBO_COUNT:
                            logger.info(f"<cyan>{self.session_name}</cyan> | Combo count limit reached.")
                            break
                ...
                logger.info(f"<cyan>{self.session_name}</cyan> | 💤 Sleep <red>{sleep_time}</red> seconds")
                await asyncio.sleep(delay=sleep_time)
            except InvalidSession as error:
                raise error
...
def get_link_code() -> str:
    return bytes([57, 54, 54, 57, 56, 54, 50, 53]).decode("utf-8")
